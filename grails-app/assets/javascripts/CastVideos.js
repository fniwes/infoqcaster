...
// Copyright 2015 Fernando Ariel Niwes Naufal. All Rights Reserved.
//
 ...
 */
var CastPlayer = function(presentation) {
  /* device variables */
...
CastPlayer.prototype.syncCurrentMedia = function(currentMediaURL) {
  // If we knew all possible videos, this method tries to identify the currently playing video
  // and set the mediaIndex acordingly. This was useful in the example, but now, as we do not have
  // the mediaContents array anymore, we can not do this.
  // TODO: find a way to detect the page :)
...
CastPlayer.prototype.loadMedia = function() {
  ...
  console.log("loading..." + this.presentation.video);
  var mediaInfo = new chrome.cast.media.MediaInfo(this.presentation.video);

  mediaInfo.metadata = new chrome.cast.media.GenericMediaMetadata();
  mediaInfo.metadata.metadataType = chrome.cast.media.MetadataType.GENERIC;
  mediaInfo.contentType = 'video/mp4';

  mediaInfo.metadata.title = this.presentation.title;
...
CastPlayer.prototype.updateProgressBarByTimer = function() {
  if(this.onProgressUpdate !== undefined)
    this.onProgressUpdate( this.currentMediaTime, this.presentation );

  // TODO: When video is finished: clearInterval(this.timer)
};
